...
RUN apk update && \
    ...
    rm -rf /usr/src/ruby && \
    cd ~/ && \
    apk add --update-cache --virtual passenger-dependencies --repository 'http://nl.alpinelinux.org/alpine/edge/testing' libexecinfo libexecinfo-dev && \
    ...
    passenger-config install-standalone-runtime --auto --connect-timeout=1 && \
